...
uint32_t Block::getBitErrorCount() {
  return bitErrorCount;
}

uint32_t Block::getErrorTableIndex() {
  return errorTableIndex;
}

void Block::updateError(uint32_t newErrorCount){
  bitErrorCount = newErrorCount;
}

void Block::updateErrorTableIndex(uint32_t newErrorTableIndex){
